...
class Ui_ImageSystem(qt.QObject):  # QObject类可以用来传递信号
    ...
    def __init__(self, parent=qt.QObject()):  # 初始化实例调用数据
        ...
        self.Tab.show()
        self.Tab.setWindowTitle('车辆车牌检测识别系统')
...
class Controller(qt.QObject):  # 子线程处理类
    ...
    def VideoProcess(self):
        ...
        self.model_path = os.getcwd() + '\\keras-yolo3\\model_data\\yolo.h5'
        self.anchors_path = os.getcwd() + '\\keras-yolo3\\model_data\\yolo_anchors.txt'
        self.classes_path = os.getcwd() + '\\keras-yolo3\\model_data\\coco_classes.txt'
        self.score = 0.3
        self.iou = 0.45
...
class YOLO(object):
    ...
    def detect_image(self, image):  # 物体检测
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            score = out_scores[i]
            if (
                    predicted_class == 'car' or predicted_class == 'truck' or predicted_class == 'bus') and score > 0.5 and (
                    box[2] - box[0]) * (
                    box[3] - box[1]) > image.size[0] * image.size[1] / 120:  # 限制条件，检测类别，概率，面积
